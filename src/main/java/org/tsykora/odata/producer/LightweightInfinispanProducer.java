 ...
 * 
 * NOTE: In producer or some context constructor, create cache according to given CacheConfigBuilder.
 *
 * @author tsykora
 */
public class LightweightInfinispanProducer implements ODataProducer {
    ...
    public EntityResponse getEntity(String entitySetName, OEntityKey entityKey, EntityQueryInfo queryInfo) {
        ...
        EdmEntitySet entitySet = getEntitySet(entitySetName); // later use name of cache in/from context
        
        // creation of returned entry is based on real entries in cache
        // if there is an entry - return it as a OEntity
        // if not, return null, or keyDoesNotExist
        
        // do it by visitor/?/transformation? from normal cache entry (it needs to be compatible, set of properties (maxidle, etc.)
        List<OProperty<?>> p = new ArrayList<OProperty<?>>();
            p.add(OProperties.string("entrykey", "key1"));
            p.add(OProperties.string("entryvalue", "value1"));
        
        // TODO: look into context cache according to entitySetName - IF found:
        // TODO + fill list of properties according to model (key, value, lifespan, maxidle etc. etc.)
        OEntity oentity = OEntities.create(entitySet, entityKey, p, null); // null - properties and links
    ...
    public EntityResponse createEntity(String entitySetName, OEntity entity) {
        ...
        // do put in the cache
        // get entry as a OEntity - which will find it in a cache 
        // TODO: do some higher transformation across MyInternalCacheEntry later
